...
face = cv2.imread("image2.jpg")
face = cv2.flip(face, 1)
body = cv2.imread("image.jpg")
...
predictor = dlib.shape_predictor(
    "../face-feature-tracking/shape_predictor_68_face_landmarks.dat"
)
...
cv2.imshow(
    "face source",
    cv2.cvtColor(
        (cv2.polylines(face_cp, [convexhull], True, (255, 255, 255), 3)),
        cv2.COLOR_BGR2RGB,
    ),
)
...
cv2.imshow("face_source_triangles", cv2.cvtColor(face_cp, cv2.COLOR_BGR2RGB))
...
cv2.imshow(
    "body",
    cv2.cvtColor(
        (cv2.polylines(body_cp, [convexhull2], True, (255, 255, 255), 3)),
        cv2.COLOR_BGR2RGB,
    ),
)
...
for triangle in indexes_triangles:
    ...
    points = np.array(
        [[pt1[0] - x, pt1[1] - y], [pt2[0] - x, pt2[1] - y], [pt3[0] - x, pt3[1] - y]],
        np.int32,
    )
    ...
    M = cv2.getAffineTransform(
        points, points2
    )  # Warps the content of the first triangle to fit in the second one
    ...
    body_new_face_rect_area_gray = cv2.cvtColor(
        body_new_face_rect_area, cv2.COLOR_BGR2GRAY
    )

    # Creates a mask
    masked_triangle = cv2.threshold(
        body_new_face_rect_area_gray, 1, 255, cv2.THRESH_BINARY_INV
    )
    dist_triangle = cv2.bitwise_and(
        dist_triangle, dist_triangle, mask=masked_triangle[1]
    )
...
cv2.imshow("body_new_face", cv2.cvtColor(body_new_face, cv2.COLOR_BGR2RGB))
...
# Blur
# body_new_face = cv2.GaussianBlur(body_new_face, (1, 1), 0)

result = cv2.add(body_maskless, body_new_face)

cv2.imshow("body_new_face", cv2.cvtColor(result, cv2.COLOR_BGR2RGB))
...
seamlessclone = cv2.seamlessClone(
    result, body, body_head_mask, center_face2, cv2.NORMAL_CLONE
)

cv2.imshow("cvtColor(seamlessclone)", cv2.cvtColor(seamlessclone, cv2.COLOR_BGR2RGB))
